...
def get_token_payload(authorization: str = Header(default="")) -> dict:
    """
    FastAPI dependency — decode and verify a Cognito access token.
    Set SKIP_AUTH=true in .env to bypass for local development.
    """
    if SKIP_AUTH:
        return {
            "sub": "dev-user",
...
def require_admin(payload: dict = Depends(get_token_payload)) -> dict:
    """Caller must be in the quickjobs-admins Cognito group."""
    if "quickjobs-admins" not in payload.get("cognito:groups", []):
        raise HTTPException(status_code=403, detail="Admin access required")
    return payload


def require_employer(payload: dict = Depends(get_token_payload)) -> dict:
    """Caller must be an employer or admin."""
    groups = set(payload.get("cognito:groups", []))
    if not groups & {"quickjobs-employers", "quickjobs-admins"}:
